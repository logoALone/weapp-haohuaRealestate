...
Page({
    ...
    if (currPage.data.cropImg) {
      ...
      wx.uploadFile({
        ...
        complete: function (e) {
          ...
          // console.log('-------------------------')
          // add += 1
          // var tempFilePaths = that.data.imagelist.concat(arr)
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        that.setData({
          ...
          imageIcon: datalist.house.userimage,
          imageIconPath: datalist.house.userimage,
          imageSelect: false,
          format: that.data.formatArr[datalist.house.propertype - 1],
          selectCity: datalist.house.areaname,
          areaname: datalist.house.areaname,
          propertype: datalist.house.propertype,
          areaflag: false,
          state: datalist.house.state,
          taglist: datalist.label,
        })
        if (datalist.house.state == 2) {
          that.setData({
            items: [
              { name: 1, value: '新房' },
              { name: 2, value: '二手房', checked: 'true'},
            ],
          })
        }
        for (index in datalist.images) {
          that.data.imagelist.push(datalist.images[index].images)
          that.data.imgPath.push(datalist.images[index].images)
        }
        for (index in datalist.label) {
          if (index == 0) {
            that.setData({
              tag1: datalist.label[index],
              tagIndex: 0
            }) 
          }else if (index == 1) {
            that.setData({
              tag2: datalist.label[index],
              tagIndex: 1
            }) 
          } else {
            that.setData({
              tag3: datalist.label[index],
              tagIndex: 2
            }) 
          }
        }
  ...
  addIcon: function () {
    var that = this
    // var imagelist = that.data.imageIcon
    // var imgArr = that.data.imageIconPath
  ...
  formSubmit: function (e) {
    ...
    if (e.detail.value.username == null || e.detail.value.username == undefined || e.detail.value.username == '') {
      ...
      if (!Boolean(commissionP)) {
        ...
        wx.request({
          url: 'https://house.haowuwang.cn/house/infa/edithouse',
          data: {
            houseid: this.data.houseid,
